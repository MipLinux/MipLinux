...
| ⑤ 安装程序（M0–M2） | [installer-roadmap.md](installer-roadmap.md) · [tech/04](tech/04-安装逻辑与实测.md) · [tech/07](tech/07-M2界面设计.md) | ✅ M0 启动链与检查点 1 / 2 实测通过（含 tty1 兜底）；✅ M1 检查点 4 由 PR #42 验过；✅ **M2 验收通过**（2026-09-26：无头驱动 `tools/gui-install.py` 在真 ISO 上点完整条链、**检查点 3 与 4 都实测过**；单测 239 全绿、接线烟测 44 项、15 个流程页一屏放得下）。真跑抓到两个缺陷并已修；**检查点 5 / 6 仍归线 H**。技术栈见 D14 |
