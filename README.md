...
| 安装程序 | 🚧 **M0 完成**、**M1 完成**、**M2 进行中**：全部流程页面（加载 / 欢迎 / 网络 / 磁盘 / 分区 / 擦除确认 / 账户 / 安装详情 / 安装 / 结束，外加高级安装四项）**已接入真实后端** —— 候选盘来自 `disk.list_candidates()`、进度来自后端事件流、语言/键盘/时区名单与校验都由后端给，界面不再吃 mock（2026-09-26）。后端单测 234 个全绿、接线烟测 33 项、15 个流程页一屏放得下；**M2 验收（QEMU 里全程图形化装完一次）未做**，09-26 实机跑的还是修复前的 ISO。技术栈见 D14，规格与逐条实测状态见 [tech/07](docs/work/tech/07-M2界面设计.md) |
